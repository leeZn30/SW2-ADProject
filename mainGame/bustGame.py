...
class BustGame(QWidget):
    ...
    def startGame(self):
        ...
        # self.u_point += self.userCardDic[user_card2]
    ...
    def userFinished(self):
        ...
        if self.user_sum > 21:
            self.gamefinish = GameFinished("u_bust", self.coin)
            self.hitButton.setDisabled(True)
            self.standButton.setDisabled(True)

        elif self.user_sum == 21:
            self.gamefinish = GameFinished("u_blackjack", self.coin)
            self.hitButton.setDisabled(True)
            self.standButton.setDisabled(True)
    ...
    def dealerFinished(self):
        ...
        if self.user_sum == self.dealer_sum:
            self.gamefinish = GameFinished("push", self.coin)
            self.hitButton.setDisabled(True)

        elif self.dealer_sum > 21:
            self.gamefinish = GameFinished("d_bust", self.coin)
            self.hitButton.setDisabled(True)

        elif self.dealer_sum == 21:
            self.gamefinish = GameFinished("d_blackjack", self.coin)
            self.hitButton.setDisabled(True)

        elif self.dealer_to_21 < self.user_to_21:
            self.gamefinish = GameFinished("d_win", self.coin)
            self.hitButton.setDisabled(True)

        elif self.dealer_to_21 > self.user_to_21:
            self.gamefinish = GameFinished("u_win", self.coin)
            self.hitButton.setDisabled(True)

class GameFinished(QWidget):

    def __init__(self, result = None, coin = 0):
    ...
    def gameFinished(self):
        ...
        if self.result == "u_bust":
            ...
            self.newGame.setText("newGame")
            self.newGame.setDisabled(True)
            ...
            self.result_coin -= self.coin * 1.5
            self.show()
            ...
            self.result_lbl.setText("Push! 동점입니다!")
            self.result_lbl.setFont(QFont("times", 50))
            self.result_lbl.setStyleSheet("Color : black")
            self.newGame.setText("newGame")
            self.coin_state.setText("+" + str(int(self.coin * 1.5)))
    ...
    def newGameClicked(self):
        self.close()
        self.game = BustGame(self.coin)
        print(self.result_coin, self.coin)
